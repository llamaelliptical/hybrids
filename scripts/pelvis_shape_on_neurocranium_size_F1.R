...
# BOAS coordinates #####
# females non-parous F1 only ####
idx_F1_fnp <- classifiers_all$Generation == "F1" &
  classifiers_all$sex == "F" &
  classifiers_all$parous.non_parous.y == "N"

classifiers_F1_fnp <- classifiers_all[idx_F1_fnp, ]

# pelvic centroid size (F1 females non-parous)
csize_pelvis_F1_fnp <- csize_pelvises[idx_F1_fnp]
names(csize_pelvis_F1_fnp) <- classifiers_F1_fnp$Spec.ID

# GPA on BOAS coords (temporary size removal)
pelvis_F1_fnp_gpa <- gpagen(
  pelvis_all$coords[, , idx_F1_fnp]
)

match(
  dimnames(pelvis_F1_fnp_gpa$coords)[[3]],
  classifiers_F1_fnp$Spec.ID
)

# regress out strain from pelvis shape
regrp <- procD.lm(
  pelvis_F1_fnp_gpa$coords ~ classifiers_F1_fnp$Strain + classifiers_F1_fnp$weight ,
  SS.type = "III"
)
summary(regrp)

pelvis_F1_regr_fnp <- residualize(
  regrp$residuals,
  pelvis_F1_fnp_gpa
)

# re-add centroid size → BOAS residuals
pelvis_F1_regr_fnp_boas <- array(
  NA,
  dim = dim(pelvis_F1_regr_fnp$coords)
)

for (i in seq_len(dim(pelvis_F1_regr_fnp$coords)[3])) {
  pelvis_F1_regr_fnp_boas[, , i] <-
    pelvis_F1_regr_fnp$coords[, , i] * csize_pelvis_F1_fnp[i]
}

dimnames(pelvis_F1_regr_fnp_boas)[[3]] <-
  dimnames(pelvis_F1_regr_fnp$coords)[[3]]

# neurocranium size (F1 females non-parous)
csize_neuro_F1_fnp <- csize_neuro[idx_F1_fnp]

# regress out strain from neuro size
regrn <- lm(
  csize_neuro_F1_fnp ~ classifiers_F1_fnp$Strain + classifiers_F1_fnp$weight 
)
summary(regrn)

# pelvis BOAS residuals ~ neuro size
regr <- procD.lm(
  pelvis_F1_regr_fnp_boas ~ regrn$residuals
)
summary(regr)


# plot the regression for females non parous F1 
strain <- as.factor(classifiers_F1_fnp$Strain)
...
plot(
  ...
  main = "Pelvis form on neuro size (F1 females, non-parous), 
...
scores <- two.d.array(pelvis_F1_regr_fnp_boas) %*% as.vector(size_vector)
...
regrsize<- lm (csize_pelvis_F1_fnp ~ classifiers_F1_fnp$weight)
summary(regrsize)

regrsize<- lm (csize_neuro_F1_fnp ~ classifiers_F1_fnp$weight)
summary(regrsize)
