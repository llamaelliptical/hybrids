...
# BOAS coordinates #####
# females non-parous B3 only ####
idx_B3_fnp <- classifiers_all$Generation == "B3" &
  classifiers_all$sex == "F" &
  classifiers_all$parous.non_parous.y == "N"

classifiers_B3_fnp <- classifiers_all[idx_B3_fnp, ]

# pelvic centroid size (B3 females non-parous)
csize_pelvis_B3_fnp <- csize_pelvises[idx_B3_fnp]
names(csize_pelvis_B3_fnp) <- classifiers_B3_fnp$Spec.ID

# GPA on BOAS coords (temporary size removal)
pelvis_B3_fnp_gpa <- gpagen(
  pelvis_all$coords[, , idx_B3_fnp]
)

match(
  dimnames(pelvis_B3_fnp_gpa$coords)[[3]],
  classifiers_B3_fnp$Spec.ID
)

# regress out strain from pelvis shape
regrp <- procD.lm(
  pelvis_B3_fnp_gpa$coords ~ classifiers_B3_fnp$Strain + classifiers_B3_fnp$weight ,
  SS.type = "III"
)
summary(regrp)

pelvis_B3_regr_fnp <- residualize(
  regrp$residuals,
  pelvis_B3_fnp_gpa
)

# re-add centroid size → BOAS residuals
pelvis_B3_regr_fnp_boas <- array(
  NA,
  dim = dim(pelvis_B3_regr_fnp$coords)
)

for (i in seq_len(dim(pelvis_B3_regr_fnp$coords)[3])) {
  pelvis_B3_regr_fnp_boas[, , i] <-
    pelvis_B3_regr_fnp$coords[, , i] * csize_pelvis_B3_fnp[i]
}

dimnames(pelvis_B3_regr_fnp_boas)[[3]] <-
  dimnames(pelvis_B3_regr_fnp$coords)[[3]]

# neurocranium size (B3 females non-parous)
csize_neuro_B3_fnp <- csize_neuro[idx_B3_fnp]

# regress out strain from neuro size
regrn <- lm(
  csize_neuro_B3_fnp ~ classifiers_B3_fnp$Strain + classifiers_B3_fnp$weight 
)
summary(regrn)

# pelvis BOAS residuals ~ neuro size
regr <- procD.lm(
  pelvis_B3_regr_fnp_boas ~ regrn$residuals
)
summary(regr)


# plot the regression for females non parous B3 
strain <- as.factor(classifiers_B3_fnp$Strain)
...
plot(
  ...
  main = "Pelvis form on neuro size (B3 females, non-parous), 
...
scores <- two.d.array(pelvis_B3_regr_fnp_boas) %*% as.vector(size_vector)
...
regrsize<- lm (csize_pelvis_B3_fnp ~ classifiers_B3_fnp$weight)
summary(regrsize)

regrsize<- lm (csize_neuro_B3_fnp ~ classifiers_B3_fnp$weight)
summary(regrsize)
